...
OPC.prototype.connect = function(){
  var self = this;
  self.opc_client = dgram.createSocket('udp4');
  self.emit('connected');

  self.opc_client.on('close', function(){
...
OPC.prototype.put_pixel = function(strip_id, pixel_index, colors){
  ...
  self.opc_client.send(message,0,message.length,self.port,self.host, function(){
    self.emit('data_sent');
  });
};
...
// console.log(self.strips[strip_id].pixels);
  var message = assemble_opc_message(self.strips[strip_id]);

  this.opc_client.send(message,0,message.length,self.port,self.address, function(){
    self.emit('data_sent');
  });
};
...
function assemble_opc_message(strip){
  ...
  //console.log(strip.hi_byte,strip.lo_byte)
